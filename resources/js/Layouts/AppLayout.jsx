...
export default function AppLayout({ children, title }) {
    const { auth, flash, headerCities, currentCityName, ziggy } = usePage().props;
    ...
    const fetchSuggestions = async (query) => {
        try {
            const response = await fetch(`/events/suggest?q=${encodeURIComponent(query)}`, {
                ...
                >
                    <div className="p-5 pb-2">
                        <Link href={route('events.index')} className="flex items-center gap-3 group" onClick={() => setSidebarOpen(false)}>
                            ...
                            <div>
                                <div className="text-[10px] text-violet-400 uppercase tracking-[0.2em] font-bold">Events</div>
                                <div className="font-extrabold text-white text-lg leading-tight">Афиша</div>
                            </div>
                        </Link>
                    </div>
                    ...
                    <div className="p-5 pt-2 border-t border-white/10">
                        ...
                        <p className="text-[10px] text-zinc-600 uppercase tracking-wider">{new Date().getFullYear()} · Афиша</p>
                    </div>
                </aside>
                            ...
                            <div className="flex items-center gap-4 flex-wrap">
                                ...
                                <form onSubmit={handleSearchSubmit} className="flex-1 min-w-0 max-w-lg relative" ref={searchRef}>
                                    ...
                                    {searchOpen && searchItems.length > 0 && (
                                        <div className="absolute left-0 right-0 top-full mt-2 rounded-xl border border-white/10 bg-[#111] shadow-2xl shadow-black/50 p-3 z-50">
                                            <p className="text-xs text-zinc-500 px-3 pb-2 uppercase tracking-wider">{searchTitle}</p>
                                            <div className="space-y-1">
                                                {searchItems.map((item) => (
                                                    <Link key={item.url} href={item.url} className="block px-3 py-2 rounded-lg hover:bg-violet-500/10 transition-colors">
                                                        <div className="text-sm text-white font-medium">{item.title}</div>
                                                        <div className="text-xs text-zinc-500">{[item.date, item.venue].filter(Boolean).join(' · ')}</div>
                                                    </Link>
                                                ))}
                                            </div>
                                        </div>
                                                    ...
                                                    <nav className="py-1">
                                                        {auth.user.is_admin && (
                                                            <Link href={route('admin.events.index')} className="flex items-center gap-3 px-4 py-2.5 text-sm text-zinc-400 hover:bg-violet-500/10 hover:text-white transition-colors">
                                                                <span className="w-5 text-center text-xs">⚙</span> Админ-панель
                                                            </Link>
                                                        )}
                    ...
                    <footer className="border-t border-white/10 py-6 mt-auto">
                        <div className="px-4 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-zinc-600">
                            <div className="flex items-center gap-3">
                                ...
                                <span className="font-bold text-zinc-400 uppercase tracking-wider">Афиша</span>
                            </div>
                            <span>{new Date().getFullYear()} · События и билеты</span>
                        </div>
                    </footer>
                </div>
            </div>
