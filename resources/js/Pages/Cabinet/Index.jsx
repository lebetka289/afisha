        ...
        <AppLayout>
            ...
            <div className="grid gap-8 lg:grid-cols-[1fr,2fr]">
                ...
                <div className="border-2 border-white/10 bg-[#0a0a0a] p-6" id="city">
                    ...
                    <p className="text-xs text-zinc-600 mb-2 font-bold uppercase tracking-wider">Город для афиши</p>
                    ...
                    {bookings.length === 0 ? (
                        ...
                        <ul className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
                            {bookings.map(booking => {
                                const isPast = new Date(booking.event?.start_at) < new Date();
                                return (
                                    <li key={booking.id} className={`border-2 overflow-hidden transition-colors ${isPast ? 'border-white/[0.05] bg-zinc-900/50' : 'border-white/10 bg-[#0a0a0a] hover:border-violet-500/30'}`}>
                                        <div className="text-xs px-4 pt-4 text-zinc-500 font-bold uppercase tracking-wider">
                                            {formatDate(booking.event.start_at)}
                                        </div>
                                        <div className={`aspect-[16/10] mt-3 overflow-hidden ${isPast ? 'grayscale opacity-60' : ''}`}>
                                            {booking.event.poster_is_video ? (
                                                <video src={booking.event.poster_src} autoPlay loop muted playsInline className="w-full h-full object-cover"></video>
                                            ) : booking.event.poster_src ? (
                                                <img src={booking.event.poster_src} alt={booking.event.title} className="w-full h-full object-cover" />
                                            ) : null}
                                        </div>
                                        <div className="p-4 space-y-2">
                                            <p className={`text-xl font-black ${isPast ? 'text-zinc-400' : 'text-white'}`}>{booking.event.title}</p>
                                            ...
                                            <p className="text-sm text-zinc-600">{booking.event.venue?.name}</p>
                                        ...
                                        <div className="px-4 pb-4 flex items-center gap-2">
                                            ...
                                            {booking.status !== 'refunded' && (
                                                <button 
                                                    onClick={() => {
                                                        if (confirm('Вернуть билет?')) {
                                                            router.post(route('cabinet.bookings.refund', booking.id));
                                                        }
                                                    }}
                                                    className="px-3 py-1.5 text-xs font-black uppercase tracking-wider bg-rose-500/10 border border-rose-500/30 text-rose-300 hover:bg-rose-500/20 hover:text-white transition-colors"
                                                >
                                                    Возврат
                                                </button>
                                            )}
