...
export default function Show({ event, isFavorited: initialIsFavorited, mapLat, mapLng }) {
    ...
    const submitBooking = (e, testMode = 0) => {
        ...
        router.post(route('events.book', event), finalData);
    };
        ...
        <AppLayout>
            ...
            <div className="grid gap-6 lg:grid-cols-[1.2fr,0.8fr]">
                <div className="space-y-6">
                    ...
                    <div className="border-2 border-white/10 bg-[#0a0a0a] p-4 sm:p-6 space-y-4" id="booking-steps">
                        ...
                        <div className="relative border-2 border-white/10 bg-black aspect-[16/9] min-h-[200px] sm:min-h-[280px] lg:min-h-[360px] overflow-hidden rounded-lg">
                            <div className="absolute right-2 top-2 sm:right-4 sm:top-4 z-20 flex flex-col gap-1 sm:gap-2">
                                <button type="button" onClick={() => setHallScale(Math.min(2, hallScale + 0.2))} className="w-9 h-9 sm:w-10 sm:h-10 bg-violet-600 text-white font-bold text-base sm:text-lg hover:bg-violet-700 transition-colors rounded touch-manipulation" aria-label="Увеличить">+</button>
                                <button type="button" onClick={() => setHallScale(Math.max(0.8, hallScale - 0.2))} className="w-9 h-9 sm:w-10 sm:h-10 bg-violet-600 text-white font-bold text-base sm:text-lg hover:bg-violet-700 transition-colors rounded touch-manipulation" aria-label="Уменьшить">−</button>
                            </div>
                            <div className="absolute inset-0 overflow-auto touch-pan-x touch-pan-y">
                                <div className="absolute inset-0 min-w-0 min-h-0 origin-center transition-transform duration-200" style={{ transform: `scale(${hallScale})` }}>
                                    <div className="absolute inset-2 sm:inset-4 border border-dashed border-white/10 rounded"></div>
                                    <div className="absolute top-2 sm:top-4 left-1/2 -translate-x-1/2 px-4 sm:px-6 py-1.5 sm:py-2 text-[10px] sm:text-xs tracking-[0.2em] font-black bg-white/10 text-zinc-400 border border-white/10 uppercase rounded">Сцена</div>
                                    {event.sections.map((section) => {
                                        const pos = section.position || { x: 15, y: 20, width: 20, height: 15 };
                                        return (
                                            <div 
                                                key={section.id}
                                                className="absolute border border-white/20 backdrop-blur cursor-pointer transition hover:scale-[1.01] active:scale-[0.99] touch-manipulation rounded"
                                                style={{
                                                    left: `${pos.x}%`, 
                                                    top: `${pos.y}%`, 
                                                    width: `${pos.width}%`, 
                                            ...
                                            >
                                                <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-2 sm:px-3 text-[10px] sm:text-xs font-bold text-white">
                                                    <span className="truncate max-w-full">{section.name}</span>
                                                    <span className="text-[10px] text-zinc-400 mt-0.5">{section.seating_mode === 'seated' ? 'Сидячие' : 'Зона'}</span>
                                                </div>
                                            </div>
                                        );
                                    })}
                    ...
                    <div className="space-y-4 sm:space-y-6">
                        {event.sections.map((section) => (
                            <div key={section.id} className="border-2 border-white/10 bg-[#0a0a0a] p-4 sm:p-5 space-y-4 rounded-lg sm:rounded-none" id={`section-${section.id}`}>
                                <div className="flex flex-wrap items-center justify-between gap-3">
                                    <div className="min-w-0">
                                        <p className="text-[10px] sm:text-xs uppercase tracking-[0.15em] text-zinc-600 font-bold">{section.type}</p>
                                        <h3 className="text-base sm:text-xl font-black text-white truncate">{section.name}</h3>
                                    </div>
                                    <div className="text-right shrink-0">
                                        <div className="text-[10px] sm:text-xs text-zinc-600 uppercase font-bold tracking-wider">Стоимость от</div>
                                        <div className="text-lg sm:text-xl font-black text-white">{section.price.toLocaleString('ru-RU')} ₽</div>
                                    </div>
                                </div>

                                {section.seating_mode === 'seated' ? (
                                    <div className="border border-white/10 bg-black/50 p-3 sm:p-4 overflow-x-auto overflow-y-hidden scrollbar-thin -mx-2 sm:mx-0 rounded-lg">
                                        <div className="space-y-2 min-w-0">
                                            {Object.entries(
                                                ...
                                                <div key={row} className="flex items-center gap-2">
                                                    <span className="w-8 sm:w-10 text-[10px] sm:text-xs text-zinc-600 text-right font-bold shrink-0">{row}</span>
                                                    <div className="seat-row-grid flex-1 min-w-0" style={{ gridTemplateColumns: `repeat(${section.cols || 1}, minmax(44px, 1fr))` }}>
                                                        {seats.sort((a,b) => a.col_number - b.col_number).map((seat) => (
                                                            <button 
                                                                ...
                                                                className={`seat-btn text-[10px] sm:text-xs font-bold border py-2 sm:py-1 min-h-[44px] sm:min-h-0 touch-manipulation rounded ${
