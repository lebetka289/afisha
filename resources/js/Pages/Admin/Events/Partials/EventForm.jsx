...
export default function EventForm({ event, venues, artists, action, method = 'POST' }) {
    ...
    const [mapLat, setMapLat] = useState(event.latitude ?? event.venue?.latitude ?? 55.7558);
    ...
    useEffect(() => {
        if (data.venue_id) {
            ...
            if (venue?.latitude != null && venue?.longitude != null && (data.latitude === '' || data.longitude === '')) {
                setMapLat(Number(venue.latitude));
                setMapLng(Number(venue.longitude));
                setData('latitude', venue.latitude);
                setData('longitude', venue.longitude);
            }
        }
    }, [data.venue_id, venues]);

    const onMapCoordsChange = (lat, lng) => {
        setMapLat(lat);
        setMapLng(lng);
    ...
    useEffect(() => {
        const show = data.venue_id || data.latitude !== '' || data.longitude !== '';
        if (!show) {
            if (mapRef.current) {
        ...
        if (!mapRef.current) {
            ...
            const markerIcon = L.divIcon({
                className: 'leaflet-marker-violet',
                html: '<div style="width:24px;height:24px;background:#7C3AED;border:2px solid #fff;border-radius:50%;box-shadow:0 2px 6px rgba(0,0,0,0.4);"></div>',
                iconSize: [24, 24],
                iconAnchor: [12, 12],
            });
            ...
            map.on('click', (e) => {
                ...
                onMapCoordsChange(e.latlng.lat, e.latlng.lng);
            });
            mapRef.current = map;
    ...
    }, [mapLat, mapLng, data.venue_id, data.latitude, data.longitude]);
                ...
                <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 space-y-4">
                    ...
                    {(data.venue_id || data.latitude !== '' || data.longitude !== '') && (
                        <div className="col-span-full space-y-2">
                            ...
                            <p className="text-[11px] text-slate-500">Клик по карте устанавливает точку. Или введите город/адрес выше.</p>
                            <div className="grid grid-cols-2 gap-3">
                                <label className="text-xs uppercase text-slate-500 tracking-wide">Широта
                                    <input type="number" step="0.0001" value={mapLat} onChange={e => onMapCoordsChange(Number(e.target.value), mapLng)}
                                        className="mt-1 w-full rounded-xl bg-slate-900 border border-slate-800 px-3 py-2 text-white text-sm" />
                                </label>
                                <label className="text-xs uppercase text-slate-500 tracking-wide">Долгота
                                    <input type="number" step="0.0001" value={mapLng} onChange={e => onMapCoordsChange(mapLat, Number(e.target.value))}
                                        className="mt-1 w-full rounded-xl bg-slate-900 border border-slate-800 px-3 py-2 text-white text-sm" />
                                </label>
                            </div>
                            <p className="text-[11px] text-slate-600">Координаты сохраняются с мероприятием и отображаются на странице события.</p>
                        </div>
                    )}
                ...
                <div className="grid gap-8 lg:grid-cols-[2fr,1fr]">
                    ...
                    <div className="bg-slate-950 border border-slate-800 rounded-2xl p-4 space-y-3">
                        <div className="grid gap-3">
                            ...
                            <label className="text-xs uppercase text-slate-500 tracking-wide">Тип билета (ТЗ: обычный / VIP / Meet&amp;Greet)
