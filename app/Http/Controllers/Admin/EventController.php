...
class EventController extends Controller
{
    ...
    {
        if (auth()->user()->isOrganizer() && ! auth()->user()->isAdmin() && (int) $event->created_by !== (int) auth()->id()) {
            abort(403, 'Вы можете редактировать только свои мероприятия.');
        }
    ...
    {
        if (auth()->user()->isOrganizer() && ! auth()->user()->isAdmin() && (int) $event->created_by !== (int) auth()->id()) {
            abort(403, 'Вы можете удалять только свои мероприятия.');
        }
        $event->delete();
    ...
    protected function validateEvent(EventRequest $request, ?int $eventId = null): array
    {
        ...
        $validated['latitude'] = isset($validated['latitude']) && $validated['latitude'] !== '' ? (float) $validated['latitude'] : null;
        $validated['longitude'] = isset($validated['longitude']) && $validated['longitude'] !== '' ? (float) $validated['longitude'] : null;
